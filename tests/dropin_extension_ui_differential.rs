...
const UI_SCENARIOS: &str = include_str!("dropin_extension_ui_differential/fixtures/g05_extension_ui_scenarios.json");
...
impl ExtensionUiDifferentialTester {
    ...
    fn validate_ui_scenario(&self, scenario: &Value, actual_responses: &Value) -> Result<bool, Box<dyn std::error::Error>> {
        let expected_patterns = scenario["expected_patterns"].as_array().expect("expected patterns");
        ...
        for pattern in expected_patterns {
            ...
            let found = responses.iter().any(|response| {
                self.matches_pattern(response, pattern, pattern_type)
            });
    ...
    fn matches_pattern(&self, response: &Value, pattern: &Value, pattern_type: &str) -> bool {
        match pattern_type {
            "extension_ui_request" => {
                ...
                pattern.get("method").map_or(true, |m| response.get("method") == Some(m)) &&
                pattern.get("has_timeout").map_or(true, |_| response.get("timeout").is_some())
            }
            "response_success" => {
...
fn canonicalize_ui_response(value: &Value) -> Value {
    match value {
        Value::Array(items) => {
            Value::Array(items.iter().map(canonicalize_ui_response).collect())
        },
        Value::Object(object) => {
...
fn g05_extension_ui_differential_fixture_validation() {
    ...
    assert_eq!(scenarios["schema"], "pi.dropin.extension_ui_differential_scenarios.v1");
    assert_eq!(scenarios["bead"], "bd-lnmtp.2.4");

    let ui_scenarios = scenarios["scenarios"].as_array().expect("UI scenarios array");
    assert!(
        ui_scenarios.len() >= 10,
    ...
    for scenario in ui_scenarios {
        ...
        assert!(scenario.get("description").is_some(), "{} missing description", id);
        assert!(scenario.get("requests").is_some(), "{} missing requests", id);
        assert!(scenario.get("expected_patterns").is_some(), "{} missing expected_patterns", id);
    }
}
...
fn g05_extension_ui_canonicalization_stable() {
    ...
    for (i, test_case) in test_cases.iter().enumerate() {
        ...
        if let Value::Object(obj) = &canonical_once {
            assert!(!obj.contains_key("timestamp"), "timestamp should be removed");
            assert!(!obj.contains_key("requestId"), "requestId should be removed");
...
fn g05_extension_ui_differential_basic_scenarios() {
    ...
    if !std::path::Path::new(&rust_pi_path).exists() {
        eprintln!("Warning: Rust pi binary not found at {}. Skipping UI differential test.", rust_pi_path);
        return;
    }
    ...
    let tester = match ExtensionUiDifferentialTester::new() {
        Ok(t) => t,
        Err(e) => {
            eprintln!("Warning: Failed to create UI differential tester: {}. Skipping test.", e);
            return;
        }
    };
    ...
    for scenario in ui_scenarios.iter().take(5) {
        ...
        match tester.execute_ui_scenario(scenario) {
            Ok(responses) => {
                match tester.validate_ui_scenario(scenario, &responses) {
                    Ok(true) => {
                        successful_scenarios += 1;
                        println!("✓ {}: {} - PASS", scenario_id, scenario_type);
                    }
                    Ok(false) => {
                        failed_scenarios.push(format!("{}: {} - Pattern mismatch", scenario_id, scenario_type));
                        println!("✗ {}: {} - FAIL", scenario_id, scenario_type);
                    }
                    Err(e) => {
                        failed_scenarios.push(format!("{}: {} - Validation error: {}", scenario_id, scenario_type, e));
                        println!("✗ {}: {} - ERROR: {}", scenario_id, scenario_type, e);
                    }
                }
            }
            Err(e) => {
                failed_scenarios.push(format!("{}: {} - Execution error: {}", scenario_id, scenario_type, e));
                println!("✗ {}: {} - ERROR: {}", scenario_id, scenario_type, e);
            }
        }
    }
    ...
    assert!(total_scenarios > 0, "Should have tested at least one UI scenario");
}